...
>
> This file is kept for **provenance only**. Its live backlog was absorbed into
> `GAME_MECHANICS_IDEAS.md` §2 (build status, with `file:line` evidence for every claim) and its
> superseded entries into §5.3. **Do not edit this file, and do not build from it** — several of its
> items have since shipped or been re-scoped, and it has no record of which.
>
> In particular it does not know that Level 6 (THE BREACH) shipped, which makes its #14/#17/#18
> entries misleading.
...
## ⚠️ 2026-07-27 — SIX ITEMS SUPERSEDED BY `SCARY.md`

`SCARY.md` is now the authoritative document for **fear craft**. It re-specs the following six items
at full design detail (they are one-paragraph sketches here) and should be read *instead of* these
entries. Annotated rather than deleted, per the `IDEA_HISTORY.md` convention — and because `KONTUR.md`
is already a cautionary tale in this repo about a stale doc silently contradicting the shipped game.

| Item below | Superseded by | Note |
|---|---|---|
| **#1 Adaptive Director pacing** | `SCARY.md` §3 P9 | **Scoped down deliberately.** `ApparitionDirector` already does the hard half correctly (randomised gap, grace, panic ceiling, UI/freeze vetoes, `OVERDUE_AFTER`). What was actually missing is the Alien: Isolation *relief* half — and `RandomAmbient`, the louder system, has no panic gate at all. Build the gate; do **not** build a second director. |
| **#2 Hide the panic meter (KONTUR)** | `SCARY.md` §3 P8 | **Reframed.** Lie about the *readout* for under a second and revert; never about the *rules*. Twelve levels of fail economy are tuned arithmetic — Amnesia could afford a placebo meter because its meter did nothing; this one does everything. See `SCARY.md` §8.6. |
| **#4 Falling/collapsing props** | `SCARY.md` §3 P6 | `MovedProp` is the better version of the same instinct: no physics, no sound, no acknowledgement, and it works *retroactively* on every prop the player already walked past. |
| **#5 Misdirection pairing** | `SCARY.md` §3 P4 + P11 | Misdirection turns out to be the *mechanism* rather than the feature — it is how the false-ceiling telegraph and the geometry mutation both work. |
| **#7 Pre-scare silence dip** | `SCARY.md` §3 P5 | Kept essentially as pitched, plus the key placement: **inside `flash_scare()` itself**, which improves every existing survivable scare in the game at once for two lines. |
| **#20 Shifting-architecture level** | `SCARY.md` §3 P11 + §5.1 | Split into a *retrofit* (the House hallway) and a *level* (THE RETURN, the P.T. loop), rather than one or the other. |

Everything else below (#3, #8, #10, #11, #12, #13, #14, #15, #16, #19) is puzzle / agency / systems
work, is **not** superseded, and remains the live menu.

`SCARY.md` also adds three new levels not pitched here (OBSERVATION, THE ANECHOIC CHAMBER, THE
RETURN), and `DUNGEON_NIGHTMARES.md` specifies a fourth (THE NIGHTMARE). All four are accepted.
