...
- **Power-restore quest**: 3 `Breaker`s (`breaker.gd`) — Exam1, Records, and BreakerNook (see below). Each `flipped` → `_on_breaker_flipped()`; the third → `_restore_power()` lifts the lamps to full and drops the `MorgueShutter` (a `CSGBox3D` gating the morgue doorway). Three difficulty tiers: Exam1 stays lit-and-obvious on its wall centre. **BUG_FIX.md 4.1**: Records' is the odd one out — moved off the doorway-facing wall onto the room's other dead-end wall (`Vector2(0,-1)`), findable by a periodic spark/flicker tell (`_tick_breaker_spark`, `breaker_spark.wav`). Records' note and warning sign live on its previously-unused north wall (their old spot became the DarkCorridor doorway)
- **The dark wing — audio-only mini-game (redesign)**: Exam2's breaker (formerly "obvious on sight") now lives in **`BreakerNook`**, reached through a genuine branching maze off Records' west wall: `DarkCorridor` (7 m straight) → `Junction` (a 4×4 three-way room) → either `DeadEnd` (a dead stub — the wrong turn) or `BreakerNook` (the real destination). All four rooms are lampless (`NO_LAMP_ROOMS`) and share one combined flashlight-lock `Area3D` (`_spawn_breaker_nook_zone()`, bounds x −28..−12 / z 6.5..14.5) that calls `player.lock_flashlight()` on entry and symmetrically `unlock_flashlight()` on exit — leaving without finding the breaker is always the safety valve. **No `DarkZone`** anywhere in the wing (would double-tax the exact posture the room's premise requires — Issue 18). The breaker itself is genuinely unseeable: `breaker.gd` gained `@export var glows: bool = true`, gating all of its emission (both the panel and the lever indicator) — emission self-illuminates regardless of scene darkness, so a normally-built breaker was still clearly visible in a "pitch black" room even with every lamp excluded. The BreakerNook instance sets `glows = false`; Exam1 and Records keep the default and stay normally lit. Findable purely by ear: a sound-only tell (`_spawn_dark_breaker_tell`/`_tick_dark_breaker_tell`, positional `breaker_spark.wav` every 8–12 s, no light component) plus a live on-screen **hot/cold proximity meter** — `panic_hud.gd:set_breaker_proximity(ratio)` (a self-built bar, cold blue-grey → hot red, hidden outside the wing), driven every frame by `level_1.gd:_tick_breaker_meter()` from `player.global_position.distance_to(_dark_breaker_pos)` over `METER_MAX_DIST=14`, reached via a new `player.gd:get_panic_hud()` getter. Deliberately **distance-only, no direction** — the meter confirms "closer/further" so the room reads as an active mini-game, while which branch to take is still a real choice driven by sound and memory. The Lab's `DEBUG_APPARITION` fatal-apparition timer is suppressed for the whole wing (`_in_breaker_nook` flag checked in `_tick_debug_apparition`) — a player who can't see the apparition materialise has no fair way to judge "hold still or flee," the same double-jeopardy mistake KONTUR Gate 7 and the Backrooms Flood already made once each
...
- **Gate 5 — THE ROSTER** (`combination_lock.gd`, *recall*): a personnel gate in Records welded shut
  ...
  touching `GameState.level2_code`. **BUG_FIX.md 3.1**: the lock plate itself was a flat untextured
  green `BoxMesh` (debug capture #12); it now carries a `kontur_lock_roster.png` corroded-lock texture
  on a `QuadMesh` face off the box's inner side, not on the box itself (Issue 24 applies here too)
...
| `panic_hud.gd` | `PanicHUD` node (loaded from `assets/elements/hud_canvas.tscn`). `set_panic_ratio(ratio)` drives blur (`BlurRect`) and red-tint (`TintRect`) shader overlays. Spawned by `player.gd` in `_ready()`, reachable via `player.gd:get_panic_hud()`. Also self-builds an objective label (`_build_objective_label`) and the Lab's dark-wing `set_breaker_proximity(ratio)` hot/cold bar (ratio<0 hides it). |
...
| `breaker.gd` | `class_name Breaker` (Session 10) — Lab power switch; `interact()` flips once + emits `flipped` + clunk (`breaker_throw`). `@export var glows: bool = true` gates all emission (panel + lever) — set `false` for BreakerNook's breaker so it stays genuinely invisible in the dark; emission otherwise self-illuminates regardless of scene darkness |
...
| `kontur_mailbox.gd` | `class_name KonturMailbox` (**BUG_FIX.md 3.2**) — KONTUR Landing's mailbox, upgraded from a flat wall decal to a real interactable; `@export hint_text`, `interact()` calls `NoteUI.show_note(hint_text)`. The level builds the mesh/collider (depth box + QuadMesh art face, same Issue-24-safe pattern), this script only owns the interaction |
| `house_map_prop.gd` | `class_name HouseMap` — the Kitchen's folded map prop; `interact()` opens its child `MazeChaseUI`, `signal won`, owns the catch consequence (`jolt_camera` + `add_panic(CATCH_PANIC)`) and a `_solved` one-shot guard, same division of labor as `key_item.gd`/`kontur_mailbox.gd` |
