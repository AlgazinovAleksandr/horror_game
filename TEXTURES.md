...
| `level_1_lab/lab_keycard.png` | Facility ID keycard, flat top-down elevation — pale plastic, KONTUR/facility branding, magnetic stripe, photo blacked out, grimy | Level 1 — the guarded morgue keycard (`level_1.gd:_spawn_morgue_keycard`), on a **top-facing quad** over the green emissive slab | to_be_added |
...
| `level_5_kontur/kontur_keycard.png` | KONTUR access card, flat front elevation — Soviet-era institutional card, Cyrillic header, subject number, punched corner, faint bloom | Level 5 — Gate 3's **bait** card on the offering pedestal (`offering_pedestal.gd` `CARD_TEX`), on a quad over the card slab. Should read as desirable — taking it forfeits the run | to_be_added |
